...
(Fig.~\ref{fig:temporal}). Training an XGBoost (kin+size) model on objects discovered
before 2015 (PHA 23.3\%) and testing on later discoveries (PHA 2.4\%) retains recall
(0.983; $F_2=0.971$) but degrades precision to 0.928 at the default threshold
(vs.\ $\approx$0.97 under random CV): the prevalence shift, not the discriminative
rule, carries the cost, consistent with the rule being essentially the era-stable
size cut. PHA status correlates with orbital arc ($r=0.65$), making the
characterization confound explicit. Conditioning the catalog on close approaches only mildly enriches
...
\subsection{MOID Prediction}
Because orbital MOID requires extended astrometric observations to establish a
full orbital solution, early-stage hazard evaluation benefits from predicting orbital
MOID directly from initial kinematic and observational features. Using the feature set
($\mathrm{distnom\_min}, v_\infty, H_\text{obs}, n_\text{appro}, \text{dist\_unc\_med}$),
gradient-boosted regression achieves $R^2 = 0.807$ and $\mathrm{MAE} = 0.0193\text{ au}$
in predicting continuous MOID, substantially outperforming the raw minimum observed distance baseline
($R^2 = 0.508, \mathrm{MAE} = 0.0259\text{ au}$). For the binary hazard threshold
($\mathrm{MOID}\le 0.05\text{ au}$), classification yields $F_2 = 0.910$ (Random Forest) and $\text{ROC-AUC} = 0.963$ (XGBoost),
boosting recall from 0.785 (naive observational proxy) to over 0.904 while
preserving strong generalizability on temporal hold-out evaluations ($R^2 = 0.806, F_2 = 0.911$ for objects
discovered $\ge 2015$).
